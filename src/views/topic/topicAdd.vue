<template>
  <div class="container">
    ...
    <el-card shadow="never">
      <el-form ref="form" label-width="100" :model="topicArr" v-if="topicArr">
        <el-form-item label="标题">
          <template>
            <span style="color: #FF0000;" class="margLeft">*</span>
            <input type="text" class="titIpt" placeholder="请输入文章标题" v-model="topicArr.title" />
          </template>
        </el-form-item>
        <el-form-item label="列表图片">
         ...
         <el-form-item label="描述">
          <template>
            <span style="color: #FF0000;" class="margLeft">*</span>
            <el-input type="textarea" v-model="topicArr.desc" class="titIpt" style="margin-bottom:15px"></el-input>
          </template>
        </el-form-item>
        <el-form-item label="上传人">
          <template>
            <span style="color: #FF0000;" class="margLeft">*</span>
            <input type="text" class="titIpt" placeholder="请输入姓名" v-model="topicArr.author" />
          </template>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    uploadSuccess(res){
      ...
      console.log(res)
      if(res.code===200){
        this.topicArr.img_url=`http://${res.data}`;
      }
    },
    onSubmit(){
      //保存
      addTopic(this.topicArr).then(()=>{
